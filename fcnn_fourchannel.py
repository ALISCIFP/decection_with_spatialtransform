...
def save_as_image( img ,iter_i,epoch_i,type):
    ...
    img3 = img[0,:,:,2]
    img3 = img3.reshape((40,40))
    img4 = img[0,:,:,3]
    img4 = img4.reshape((40,40))
    
    contenated_image1 = np.concatenate((img1,img2), axis=0)
    contenated_image2 = np.concatenate((img3,img4), axis=0)

    contenated_image = np.concatenate((contenated_image1,contenated_image2), axis=1)
    ...
    return 
# %% Load data
mnist_cluttered = np.load('./data/mnist_sequence1_sample_5distortions5x5_4channel.npz')
print 'data_load'
...
Y_train = dense_to_one_hot(y_train, n_classes=37)
Y_valid = dense_to_one_hot(y_valid, n_classes=37)
Y_test = dense_to_one_hot(y_test, n_classes=37)
...
x_tensor = tf.reshape(x, [-1, 80, 80, 1])
...
h_fc_loc11 = nn_layer(x, 6400, ful_length, 'fc_loc11',act = tf.nn.tanh,method = 'zeros')
...
with tf.name_scope('fc_loc32'):
    with tf.name_scope('weights'):
        W_fc_loc32 = weight_variable([ful_length, 6],method = 'zeros')
        variable_summaries(W_fc_loc32, 'fc_loc32' + '/weights')
...
# Use identity transformation as starting point
    with tf.name_scope('biases'):
        ...
        initial = initial.astype('float32')
        initial = initial.flatten()
        b_fc_loc32 = tf.Variable(initial_value=initial, name='b_fc_loc32')
        variable_summaries(b_fc_loc32, 'fc_loc32' + '/biases')

    with tf.name_scope('Wx_plus_b'):
      fc_loc32_preactivate = tf.matmul(h_fc_loc11_drop, W_fc_loc32) + b_fc_loc32
      tf.histogram_summary('fc_loc32' + '/pre_activations',fc_loc32_preactivate)     
    h_fc_loc32 = tf.nn.tanh(fc_loc32_preactivate, 'activation')
    tf.histogram_summary('fc_loc32' + '/activations', h_fc_loc32)  
    
with tf.name_scope('fc_loc42'):
    with tf.name_scope('weights'):
        W_fc_loc42 = weight_variable([ful_length, 6],method = 'zeros')
        variable_summaries(W_fc_loc42, 'fc_loc42' + '/weights')
...
# Use identity transformation as starting point
    with tf.name_scope('biases'):
        ...
        initial = initial.astype('float32')
        initial = initial.flatten()
        b_fc_loc42 = tf.Variable(initial_value=initial, name='b_fc_loc42')
        variable_summaries(b_fc_loc42, 'fc_loc42' + '/biases')

    with tf.name_scope('Wx_plus_b'):
      fc_loc42_preactivate = tf.matmul(h_fc_loc11_drop, W_fc_loc42) + b_fc_loc42
      tf.histogram_summary('fc_loc42' + '/pre_activations',fc_loc42_preactivate)     
    h_fc_loc42 = tf.nn.tanh(fc_loc42_preactivate, 'activation')
    tf.histogram_summary('fc_loc42' + '/activations', h_fc_loc42)  
...
h_trans_2 = transformer(x_tensor, h_fc_loc32, out_size)
h_trans_3 = transformer(x_tensor, h_fc_loc42, out_size)
h_trans = tf.concat(3, [h_trans_0, h_trans_1,h_trans_2,h_trans_3])
...
h_conv2_flat = tf.reshape(h_trans, [-1, 6400])
h_fc1 = nn_layer(h_conv2_flat, 6400, 512, 'fcnn_1')
...
y_logits = nn_layer(h_fc2_drop, 512, 37, 'fcnn_3',act=None)
...
grads = opt.compute_gradients(cross_entropy, [b_fc_loc12,b_fc_loc22,b_fc_loc32,b_fc_loc42])
...
for epoch_i in range(n_epochs):
    for iter_i in range(iter_per_epoch - 1):
        ...
        if iter_i % 1000 == 0:
            ...
            img = np.concatenate((img[0:1,0:40,0:40,0:1],img[0:1,0:40,40:80,0:1],img[0:1,40:80,0:40,0:1],img[0:1,40:80,40:80,0:1]), axis=3)
