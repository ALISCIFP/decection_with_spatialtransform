...
def combined_data(x,y):
	...
	for i in range(0,(n_out-1)):
		rand1 = int(np.random.rand()*x.shape[0])
 		...
 		new_x[i,:] =  local_matr.reshape(3200)
 		new_y[i] = y[rand1] + y[rand2]
 		if i == 1:
...
np.savez('./data/mnist_sequence1_sample_5distortions5x5_2channel.npz', X_train = new_x_train,
		 y_train = new_y_train,
         X_valid = new_x_valid,
		 y_valid = new_y_valid,
		 X_test = new_x_test,
